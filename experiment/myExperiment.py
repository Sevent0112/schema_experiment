...
def execute(sql: str):
    ...
    with open("./customer/bcnf_cost.txt", "a", encoding='utf-8') as f:
        f.write(line1 + '\n')
    cur.close()
...
def getInput() -> List[str]:
    input = []
    with open('./customer/customer_query.txt', "r", encoding='utf-8') as f:
...
if __name__ == '__main__':
    columns = ['c_w_id', 'c_d_id', 'c_id', 'c_discount', 'c_credit', 'c_last',
               'c_first', 'c_credit_lim', 'c_balance', 'c_ytd_payment', 'c_payment_cnt',
               'c_delivery_cnt', 'c_street_1', 'c_street_2', 'c_city', 'c_state', 'c_zip',
               'c_phone', 'c_since', 'c_middle', 'c_data']
    table_list = [Table(1, "cus_a", [15, 1, 3, 6, 17, 18, 16, 0, 12, 14, 13, 2, 5, 20], [6], []),
                  Table(2, "cus_b", [15, 3, 8, 16], [15, 3, 16], []),
                  Table(3, "cus_c", [2, 1, 3, 4], [2, 1, 3], []),
                  Table(4, "cus_d", [10, 3, 8], [3, 8], []),
                  Table(5, "cus_e", [11, 19, 9, 7, 8], [8], [])]
